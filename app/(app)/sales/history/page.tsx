...
export default async function SalesHistoryPage({ searchParams }: { searchParams: Promise<{ q?: string; batch?: string }> }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-semibold">{t(messages, "pg.sales.history.title")}</h1><p className="mt-1 text-sm text-stone-500">{t(messages, "pg.sales.history.subtitle")}</p></div><Link href="/sales/history/trace" className="rounded-md border border-stone-300 bg-white px-4 py-2 text-sm text-teal-800 hover:bg-stone-50">{t(messages, "pg.sales.history.batchTrace")}</Link></div>
    <Card><CardHeader><h2 className="font-semibold">{t(messages, "pg.sales.history.productBatchTrace")}</h2></CardHeader><CardBody>
      <form className="flex flex-wrap gap-3"><Input name="q" defaultValue={q} placeholder={t(messages, "pg.sales.history.productSearchPlaceholder")} className="max-w-xs" /><Input name="batch" defaultValue={batch} placeholder={t(messages, "pg.sales.history.lotPlaceholder")} className="max-w-xs" /><Button type="submit">{t(messages, "pg.sales.history.search")}</Button></form>
    </CardBody></Card>
