...
export default async function PickListsPage() {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-semibold">{t(messages, "pg.warehouse.picklistsTitle")}</h1><p className="mt-1 text-sm text-stone-500">{t(messages, "pg.warehouse.picklistsSubtitle")}</p></div>
    <Card><CardHeader><h2 className="font-semibold">{t(messages, "pg.warehouse.ordersToGenerate")}</h2></CardHeader><CardBody className="flex flex-wrap gap-2">
      ...
      {!orders?.length && <span className="text-sm text-stone-400">{t(messages, "pg.warehouse.noOrdersToGenerate")}</span>}
    </CardBody></Card>
    <Card><CardHeader><h2 className="font-semibold">{t(messages, "pg.warehouse.pickList")}</h2></CardHeader><CardBody className="space-y-3">
      {(picks ?? []).map((pick) => {
        ...
        return <div key={pick.id} className="grid items-center gap-3 rounded border border-stone-100 p-3 md:grid-cols-[1fr_auto_360px]">
          ...
          {!["shipped", "cancelled"].includes(pick.status) && <form action={withdrawPickList.bind(null, pick.id)} className="flex gap-2"><Input name="reason" required placeholder={t(messages, "pg.warehouse.withdrawReason")} /><Button type="submit" variant="danger" size="sm">{t(messages, "pg.warehouse.withdrawAndUnlock")}</Button></form>}
        </div>;
      })}
    </CardBody></Card>
  </div>;
