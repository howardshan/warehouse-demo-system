...
export default async function PendingWeightPage() {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-semibold">{t(messages, "pg.warehouse.pendingWeightTitle")}</h1><p className="mt-1 text-sm text-stone-500">{t(messages, "pg.warehouse.pendingWeightSubtitle").replace("{x}", String(threshold))}</p></div>
    <Card><CardHeader><h2 className="font-semibold">{t(messages, "pg.warehouse.backlogPickLists")}</h2></CardHeader><CardBody className="space-y-3">
      {(picks ?? []).map((pick) => {
        ...
        return <div key={pick.id} className="flex items-center justify-between rounded border border-stone-100 p-4">
          ...
          <Badge tone={hours >= threshold ? "danger" : "warn"}>{t(messages, "pg.warehouse.hoursSuffix").replace("{x}", hours.toFixed(1))}</Badge>
        </div>;
      })}
      {!picks?.length && <p className="text-sm text-stone-400">{t(messages, "pg.warehouse.noPendingWeight")}</p>}
    </CardBody></Card>
  </div>;
}
