...
export default async function ShippingPage() {
  ...
  const [{ data: picks }, { data: shipping }] = await Promise.all([
    ...
    supabase.from("shipping_lists").select("id, sl_number, status, invoice_status, created_at, pick_list_id, sales_orders(so_number, customer_name_snapshot)")
      .order("created_at", { ascending: false }),
  ]);
  // 已建过发运单的拣货单不再出现在「待建」列表（一拣货单一发运单，唯一约束）
  const shippedPickIds = new Set((shipping ?? []).map((s) => s.pick_list_id));
  const pending = (picks ?? []).filter((p) => !shippedPickIds.has(p.id));
  return <div className="space-y-6">
    ...
    <Card><CardHeader><h2 className="font-semibold">{t(messages, "pg.warehouse.shippingListsToCreate")}</h2></CardHeader><CardBody className="flex flex-wrap gap-2">
      ...
      {!pending.length && <span className="text-sm text-stone-400">{t(messages, "pg.warehouse.noPicksToShip")}</span>}
    </CardBody></Card>
