...
export default async function CreditNoteQueuePage() {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-semibold">{t(messages, "pg.finance.creditNoteQueueTitle")}</h1><p className="mt-1 text-sm text-stone-500">{t(messages, "pg.finance.creditNoteQueueHint")}</p></div>
    <div className="overflow-hidden rounded-lg border border-stone-200 bg-white"><table className="w-full text-left text-sm">
      <thead className="bg-stone-50 text-stone-500"><tr><th className="px-4 py-3">{t(messages, "pg.finance.returnNote")}</th><th className="px-4 py-3">{t(messages, "pg.finance.customer")}</th><th className="px-4 py-3">{t(messages, "pg.finance.reason")}</th><th className="px-4 py-3 text-right">{t(messages, "pg.finance.creditAmount")}</th><th className="px-4 py-3">{t(messages, "pg.finance.status")}</th></tr></thead>
      <tbody>{(rows ?? []).map((row) => {
        const customer = customerMap.get(row.customer_id);
        return <tr key={row.return_note_id} className="border-t border-stone-100"><td className="px-4 py-3"><Link href={`/returns/return-notes/${row.return_note_id}`} className="font-mono text-teal-800 hover:underline">{String(row.return_note_id).slice(0, 8)}</Link></td><td className="px-4 py-3">{customer?.code} · {customer?.name}</td><td className="px-4 py-3">{row.reason}</td><td className="px-4 py-3 text-right font-semibold tabular-nums">{formatMoney(Number(row.credit_amount))}</td><td className="px-4 py-3"><Badge tone="warn">{t(messages, "pg.finance.pendingCredit")}</Badge></td></tr>;
      })}{!rows?.length && <tr><td colSpan={5} className="px-4 py-8 text-center text-stone-400">{t(messages, "pg.finance.creditNoteQueueEmpty")}</td></tr>}</tbody>
    </table></div>
  </div>;
}
